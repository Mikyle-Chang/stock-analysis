...
if st.sidebar.button('🚀 啟動全方位分析', type="primary"):
    ...
    with tab5:
        ...
        with col1:
            ...
            ax.set_xlabel("風險 (年化波動)"); ax.set_ylabel("報酬 (年化報酬)")
            ax.set_xlim(left=0)
        ...
        with col2:
            st.write("**最佳資產配置比例**")
            df_weights = pd.DataFrame({'資產': returns.columns, '比例': all_weights[tidx, :] * 100})

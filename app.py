...
if st.sidebar.button('🚀 啟動全方位分析', type="primary"):
    ...
    with tab5:
        st.subheader("⚖️ 最佳投資組合配置 (僅限台股)")
        # 關鍵修正：僅篩選台股進行優化
        tw_assets = [s for s in returns.columns if s in tw_list or s == '0050']
        if len(tw_assets) < 2:
            st.warning("台股數量不足（需至少2檔），無法進行效率前緣分析。")
        else:
            ...
            with col1:
                st.write(f"**台股組合效率前緣 (最佳夏普值: {sim_res[2, tidx]:.4f})**")
            ...
            with col2:
                ...
                ax_pie.pie(df_weights['比例'], labels=df_weights['資產'], autopct='%1.1f%%')
                st.pyplot(fig_pie); st.dataframe(df_weights.style.format({'比例': '{:.2f}%'}))
